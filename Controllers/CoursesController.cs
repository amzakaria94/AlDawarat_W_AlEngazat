...
namespace AlDawarat_W_AlEngazat.Controllers {
	...
	public class CoursesController : Controller {
		...
		public async Task<IActionResult> Index(string searchQuery, string searchLocation, string searchStartDate, string searchEndDate, string employeeCategory, int? pageNumber) {
			...
			var courseList = await courses.AsNoTracking().ToListAsync();
			ViewBag.CourseDaysLeft = courseList.Select(c => (c.EndDate - DateTime.Now).Days).ToList();
		...
		public async Task<IActionResult> DeleteConfirmed(int id) {
			...
			if (course != null) {
				// Set CourseID to null for all employees referencing this course
				foreach (var employee in course.Employees) {
